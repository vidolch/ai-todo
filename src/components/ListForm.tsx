...
export function ListForm({ list, onSubmit, onCancel, isEditing = false }: ListFormProps) {
  ...
  useEffect(() => {
    if (list) {
      ...
      if (isEditing && list.id) {
        fetchListUsers(list.id);
      }
    }
  }, [list, isEditing]);

  const fetchListUsers = async (listId: string) => {
    setIsLoadingUsers(true);
    try {
      const response = await fetch(`/api/lists/${listId}/users`);
      if (response.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-white/5 border border-white/10 rounded-lg">
      ...
      <div className="space-y-2">
        ...
        {isLoadingUsers && (
          <div className="mt-1 text-sm text-gray-400">Loading users...</div>
        )}
